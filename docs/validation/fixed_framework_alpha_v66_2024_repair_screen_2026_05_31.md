...
1. reduce the January damage by using a less aggressive warmup/confirmation
   rule or a separate January stress state;
2. test a schedule driven by top-score basket loss instead of each factor leg's
   standalone rolling health;
