...
def test_tree_score_target_builder_can_force_old_source_holdings_to_exit(tmp_path) -> None:
    ranked = pd.DataFrame(
        [
            {
                "signal_time": "t0",
    ...
    assert by_time == {"t0": ["inst-old"], "t1": ["inst-new"]}
    assert result.diagnostics["source_transition_forced_exit_count"].tolist() == [0, 1]
